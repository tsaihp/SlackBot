...
def report_to_weeklyreport_system(user, date_list, halfday):
    ...
    for x in date_list:
        ...
        config_content = "Take off in %s/%s/%s by SlackBot" % (x['date'].year,
                                                               x['date'].month,
                                                               x['date'].day)
        work_time = 20
        if x['time'] != 'wholeday':
        ...
        params['sendParam[4]'] = 'tasks:>'
        params['sendParam[4]'] += sample_content.format(
        ...
        r = s.post(
            'http://172.16.83.193/weekly/sql/db_insert_task.php', data=params)
        if r.status_code is not 200:
            print(r.json())
        else:
            print("[Weekly Report]user:%s"
                  ", take off:%s" % (user, x['date'].isoformat()))
...
def on_message(ws, message):
    ...
    if r_type == "message":
        ...
        if (r_msg["channel"][0] == "C" and
                r_msg["text"].find("<@U26SFGPP1>") == -1):
            # Not for me
            return
        ...
        if input_msg == "hello" or input_msg == "hi":
            ...
            if firstDate:
                reply_msg = "你將於%d/%d開始休假, 祝休假愉快!" % (
                    firstDate['date'].month, firstDate['date'].day)
            else:
        ...
        print("Error (%d)! %s" % (
            r_msg['error']['code'], r_msg['error']['msg']))
        return
...
def post_message(post_to, message):
    ...
    r = requests.get(
        'https://slack.com/api/chat.postMessage', params=msg_payload)
    print(r.json())
